...
w('remembered rather than sounded out. These are tagged **Red word (irregular)**')
w('instead of the phonics category they would otherwise fall in, so a student who')
w('misses one does not lose credit for a skill they may well have: in *their*, a')
w('miss on the `eir` no longer counts against r-controlled vowels, and the `th`')
w('beside it still counts as a consonant digraph.')
w()
...
w()
w('The list does not have to be complete. A pair that is missing simply behaves as')
w('it did before, so add to it as words come up in real tests.')
w()
