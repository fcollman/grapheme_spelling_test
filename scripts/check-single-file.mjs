...
/**
 * Finds references the browser would actually fetch.
 *
 * Only the markup is searched. Inlined scripts are skipped deliberately: minified
 * libraries contain strings like `<a href="${url}">` that are code for building
 * HTML at runtime, not requests this page makes. Scanning them produced a false
 * failure. Opening tags are kept, so a real `<script src=...>` is still caught.
 */
export function findExternalReferences(html) {
  ...
  return [
    ...new Set(
      [...markup.matchAll(/<(?:script|link|img|iframe|source|audio|video)\b[^>]*?\s(?:src|href)\s*=\s*["']([^"']+)["']/gi)]
        .map((m) => m[1])
...
function run() {
  ...
  if (entries.includes('index.html')) {
    ...
    if (external.length > 0) {
      problems.push(`index.html references external files: ${external.join(', ')}`)
    }
...
// Only run the check when invoked directly, so tests can import the matcher.
